...
- `src/pages/` — `Home.tsx` (wires everything together; AR renders as soon as GPS+camera+basic compass fix exist, without waiting for full calibration), `PlaceTurbines.tsx` (route `/placera`, the permission-free "Placera vindkraftverken själv" tool).
