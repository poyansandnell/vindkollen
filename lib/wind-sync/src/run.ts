...
export interface WindSyncCountrySummary {
  countryCode: string;
...
export interface WindSyncResult {
  startedAt: Date;
  finishedAt: Date;
  countries: WindSyncCountrySummary[];
}

async function syncCountry(
  adapter: CountryWindDataAdapter,
  log: (msg: string) => void,
): Promise<WindSyncCountrySummary> {
  ...
  return {
    ...
    projectAreaCount: allProjectAreas.length,
    turbineCount: allTurbines.length,
  };
}

/**
 * Run the full wind data sync for every supported country adapter. Safe to
 * call repeatedly (e.g. from a scheduler) — every write is an upsert keyed
 * on stable external IDs, so re-running just refreshes existing rows and
 * adds any newly discovered localities/turbines/project areas.
 */
export async function runWindSync(options?: { log?: (msg: string) => void }): Promise<WindSyncResult> {
  const startedAt = new Date();
  const countries: WindSyncCountrySummary[] = [];
  for (const adapter of ADAPTERS) {
    const log = (msg: string) =>
      (options?.log ?? console.log)(`[wind-sync:${adapter.countryCode}] ${msg}`);
    countries.push(await syncCountry(adapter, log));
  }
  return { startedAt, finishedAt: new Date(), countries };
}
